...
The app uses an eRepublik-style skin: light grey page, white cards, a dark navy header with the player HUD and
the green-accented menubar (`Er` tokens in `lib/core/theme.dart`, `GymTitle`/`GymTabs`/`GlowButton` in
`lib/widgets/ui.dart`); the gym scene itself stays dark; the player HUD shows the avatar with the XP ring, rank insignia, XP/wellness bars and money.
